"""
PostgreSQL adapter - types and validation.
...
class PostgresSSHConfig(BaseModel):
...
class PostgresDatabaseCredentials(BaseModel):
    """Database-level credentials for PostgreSQL."""
    hostname: str
    port: int = 5432
    database: str = "postgres"
    ssl_mode: str = SSLMode.PREFER.value
    ssh: Optional[PostgresSSHConfig] = None


class PostgresAccountCredentials(BaseModel):
    """Account-level credentials (DB user) for PostgreSQL."""
    username: str
    password: str
...
class PostgresAdapter(BaseAdapter):
    ...
    def database_type(self) -> str:
        return DatabaseType.POSTGRES.value

    def validate_database_credentials(self, credentials: dict[str, Any]) -> tuple[bool, str | None]:
        """Validate PostgreSQL database credential structure."""
    ...
    def validate_account_credentials(self, credentials: dict[str, Any]) -> tuple[bool, str | None]:
        """Validate PostgreSQL account credential structure."""
