...
function buildEstimatedCountQuery(params: EstimatedCountParams): EstimatedCountResult {
  ...
  // Fast estimated count using information_schema statistics
  const sql = `
SELECT table_rows as count
FROM information_schema.TABLES
WHERE TABLE_SCHEMA = ${escapeValue(schema)}
  AND TABLE_NAME = ${escapeValue(table)}
`.trim()
...
function buildFilterCondition(filter: FilterInput): string {
  ...
  switch (filter.operator) {
    ...
    case 'icontains':
      ...
      // but we'll use LOWER() to be explicit and safe
      return `LOWER(${quotedColumn}) LIKE LOWER(${escapeLikeValue(value)})`
    case 'startsWith':
      return `${quotedColumn} LIKE ${escapeValue(value.replace(/'/g, "''").replace(/\\/g, '\\\\') + '%')}`
    case 'endsWith':
      return `${quotedColumn} LIKE ${escapeValue('%' + value.replace(/'/g, "''").replace(/\\/g, '\\\\'))}`
    case 'isNull':
