...
export function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const redirect = searchParams.get('redirect')
      await startGoogleAuth({ redirect, inviteCode })
    } catch (err) {
      ...
      {!codeSent ? (
        <form onSubmit={handleRequestCode} className="grid w-full grid-cols-1 gap-6">
          <div className="space-y-2">
            <Heading>Sign in to VaultSQL</Heading>
          </div>
          ...
          <Button type="submit" className="w-full" disabled={requestCode.isPending}>
            {requestCode.isPending ? 'Sending code...' : 'Send login code'}
          </Button>

          <Text>
            Don't have an account?{' '}
            <TextLink href={inviteCode ? `/signup?invite_code=${inviteCode}` : '/signup'}>
